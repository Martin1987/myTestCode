...
-- cube on sales
-- demo use of GROUPING SETS to replace CUBE, HAVING and GROUPING_ID
...
group by prod_category, grouping sets(
  rollup(prod_category), -- sub total by product category
  (cust_income_level),  -- aggregate by category and income levels only
  (age_range),          -- aggregate by category and age only
  (cust_income_level,age_range) -- aggregates by category, all age and income
)
--having group_id() < 1
